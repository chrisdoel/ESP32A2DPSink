...
void A2DPSink::bt_av_new_track(void)
{
...
void A2DPSink::bt_av_playback_changed(void)
{
    if (esp_avrc_rn_evt_bit_mask_operation(ESP_AVRC_BIT_MASK_OP_TEST, &s_avrc_peer_rn_cap,
...
void A2DPSink::bt_av_play_pos_changed(void)
{
    if (esp_avrc_rn_evt_bit_mask_operation(ESP_AVRC_BIT_MASK_OP_TEST, &s_avrc_peer_rn_cap,
...
void A2DPSink::bt_av_notify_evt_handler(uint8_t event_id, esp_avrc_rn_param_t *event_parameter)
{
    switch (event_id) {
...
void A2DPSink::bt_i2s_driver_install(void)
{
    i2s_driver_install(i2sPort, &i2sConfig, 0, NULL);
    i2s_set_pin(i2sPort, &i2sPins);
}

void A2DPSink::bt_i2s_driver_uninstall(void)
{
    i2s_driver_uninstall(i2sPort);
}

void A2DPSink::volume_set_by_controller(uint8_t v)
{
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set by remote controller to: %d%%", (uint32_t)v * 100 / 0x7f);
...
void A2DPSink::volume_set_by_local_host(uint8_t v)
{
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set locally to: %d%%", (uint32_t)v * 100 / 0x7f);
    /* set the volume in protection of lock */
    _lock_acquire(&volume_lock);
    volume = v;
    _lock_release(&volume_lock);
...
void A2DPSink::bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
...
void A2DPSink::bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
...
void A2DPSink::bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
...
void A2DPSink::bt_app_a2d_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    switch (event) {
    case ESP_A2D_CONNECTION_STATE_EVT:
    case ESP_A2D_AUDIO_STATE_EVT:
...
void A2DPSink::bt_app_rc_tg_cb(esp_avrc_tg_cb_event_t event, esp_avrc_tg_cb_param_t *param)
{
    switch (event) {
    case ESP_AVRC_TG_CONNECTION_STATE_EVT:
    case ESP_AVRC_TG_REMOTE_FEATURES_EVT:
